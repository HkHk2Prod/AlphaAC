...
| [`02_annotate_dataset.ipynb`](02_annotate_dataset.ipynb) | Pulls the grown dataset **read-only** from the bucket and runs `aczero dataset annotate` for each move set in `ANNOTATE_MOVESETS` (default `universal` + `strict-ac`), warm-starting from any existing annotation file. Designed to run **concurrently** with generation, in a separate session — it never modifies or re-uploads the group dataset. | `train_rank<N>.<moveset>.annotations.json` per configured move set |
