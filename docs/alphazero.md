...
The repository ships a runnable CPU policy/value training loop. Self-play
collects MCTS visit-count policy targets and return-to-go value targets into a
replay buffer, and the configured architecture is optimized by exact gradient
descent before a fixture certificate is solved and independently verified.
